...
const AdminDashboard = () => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'SIGNED_OUT' || !session) {
        ...
        const isAdmin = session.user?.app_metadata?.is_admin ||
          ...
          session.user?.email === 'crochetella19@gmail.com';
        if (!isAdmin) {
          navigate('/');
        }
      }
  ...
  const checkAuth = async () => {
    ...
    const isAdmin = session.user?.app_metadata?.is_admin || session.user?.user_metadata?.is_admin || session.user?.email === 'crochetella19@gmail.com';
    if (!isAdmin) {
